...
function isValidPlacement(board, row, col, size, orientation) {
    ...
    if (orientation === 'horizontal') {
        return Array.from({ length: size + 2 }).every((_, i) => 
            isInsideBoard(row, col + i - 1) && isCellEmpty(row, col + i - 1) && isNearbyEmpty(row, col + i - 1)
        );
    } else {
        return Array.from({ length: size + 2 }).every((_, i) =>
            isInsideBoard(row + i - 1, col) && isCellEmpty(row + i - 1, col) && isNearbyEmpty(row + i - 1, col)
        );
    }
}
